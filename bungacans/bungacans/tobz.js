...
module.exports = tobz = async (tobz, message) => {
    try {
        ...
        const ownerNumber = '18623216664@c.us'
        ...
        // FUNCTION
                ...
                if(body === '#mute' && isMuted(chatId) == true){
                    if(isGroupMsg) {
                        if (!isAdmin) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh admin FADHIL GANS BOT!', id)
                ...
                if (body === '#unbanchat') {
                    if (!isOwner) return tobz.reply(from, 'Maaf, perintah ini hanya dapat dilakukan oleh Owner FADHIL GANS BOT!', id)
        ...
        case '#banchat':
            ...
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner FADHIL GANS BOT!', id)
...
${desc}`)
        ...
        case '#owner':
        case '#creator':
            tobz.sendContact(chatId, `18623216664@c.us`)
        ...
        case '#simi':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Admin FADHIL GANS BOT!', id) // Hanya Admin yang bisa mengaktifkan
            if (args.length === 1) return tobz.reply(from, 'Pilih enable atau disable!', id)
            if (args[1].toLowerCase() === 'enable') {
                simi_.push(chat.id)
...
🌐️ *URL*: ${url}`
        ...
        case '#nhentai': // SEARCH NHENTAI
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            tobz.reply(from, 'PREMIUM COMMAND, HUBUNGI : wa.me/18623216664', id)
          break
...
➸ *Wilayah:* +${region_name}`
        ...
        // ADMIN & OWNER
        case '#bc':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Owner FADHIL GANS BOT!', id)
            let msg = body.slice(4)
            const chatz = await tobz.getAllChatIds()
            for (let ids of chatz) {
                var cvk = await tobz.getChatById(ids)
                if (!cvk.isReadOnly) await tobz.sendText(ids, `*[ FADHIL GANS BOT TELAH ON ]*\n\n${msg}`)
            }
        ...
        case '#ownergroup':
            ...
            break
        case '#otagall': // FOR OWNER & ADMIN FADHIL GANS BOT
        case '#omentionall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Owner FADHIL GANS BOT', id)
        ...
        case '#okickall':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', id)
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya untuk Admin FADHIL GANS BOT', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case '#oadd':
            ...
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin FADHIL GANS BOT', id)
            if (!isBotGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
            try {
        ...
        case '#oleave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isOwner, !isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin FADHIL GANS BOT', id)
            await tobz.sendText(from,'FADHIL GANS BOT DIPERINTAHKAN KELUAR OLEH OWNER!!').then(() => tobz.leaveGroup(groupId))
            break
        case '#leave':
            if (!isGroupMsg) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan dalam group', id)
            if (!isGroupAdmins) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin group', id)
        ...
        case '#getses':
            ...
            break
        case '#fadhiladmin':
            let admn = `This is list of FADHIL GANS BOT Admin\nTotal : ${adminNumber.length}\n`
        ...
        case '#addadmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner FADHIL GANS BOT!', id)
                ...
                tobz.reply(from, 'Success Menambahkan Admin FADHIL GANS BOT!', id)
                }
            break
        case '#deladmin':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner FADHIL GANS BOT!', id)
                ...
                fs.writeFileSync('./lib/database/admin.json', JSON.stringify(adminNumber))
                tobz.reply(from, 'Success Menghapus Admin FADHIL GANS BOT!', id)
            break
        case '#block':
            if (!isOwner) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh Owner FADHIL GANS BOT!', id)
            for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '#unblock':
            ...
            break
        case '#ban':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya bisa di gunakan oleh admin FADHIL GANS BOT!', id)
                for (let i = 0; i < mentionedJidList.length; i++) {
        ...
        case '#help':
            ...
            break
        case '#fadhilgroup':
            tobz.reply(from, `Link Group FADHIL GANS BOT : https://chat.whatsapp.com/J711nA6Dzk7AZzPYkscvlZ\nJangan Lupa Join Ya Kak ${pushname}`, id)
            break
        case '#groupmenu':
        ...
        case '#iklan':
            ...
            break
        case '#adminmenu':
            if (!isAdmin) return tobz.reply(from, 'Perintah ini hanya untuk Admin FADHIL GANS BOT', id)
        ...
        case '#snk':
            ...
            break
        default:
            if (!isGroupMsg) return tobz.reply(from, 'Jika Ingin Menggunakan Bot Harap Masuk Ke Dalam Grup FADHIL GANS BOT, Link Ada Di Bio atau Bisa Mengetik #fadhilgroup!\nJika Ingin Sewa Bot atau Bikin Bot Harap Ketik *#iklan*', id)
            if (command.startsWith('#')) {
